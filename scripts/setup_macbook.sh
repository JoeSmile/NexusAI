...
echo "   需要配置："
echo "   - LLM_API_KEY (模型 API 密钥)"
echo ""
echo "2. 启动 PostgreSQL + pgvector："
echo "   ${YELLOW}make up${NC}  # docker compose 启动,自动创建 contextgate 库并启用 vector 扩展"
echo ""
echo "3. 安装 Python 依赖："
...
echo ""
echo "4. 初始化数据库表结构："
echo "   ${YELLOW}make db-init${NC}  # alembic upgrade head"
echo ""
echo "5. 写入开发数据（可选）："
echo "   ${YELLOW}make seed${NC}  # API Key + 示例数据"
echo ""
echo "6. 初始化 RAG 知识库（可选）："
echo "   ${YELLOW}uv run python scripts/init_rag_knowledge.py${NC}"
echo ""
echo "7. 启动服务："
echo "   ${YELLOW}make run${NC}  # 后端"
echo ""
