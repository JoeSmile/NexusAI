...
# ── 0. 前置: 重置 + 初始化示例 + 上传 COMPLIANCE ──
say ""
say "[0] 准备知识库(reset → init/sample → upload)"
...
curl -s -o /dev/null -X POST "$BASE/api/rag/upload/pdf" -H "X-API-Key: $KEY" -F "file=@docs/COMPLIANCE.md"
...
L2H0=$(dget /tmp/rq_s0.$$ cache 2>/dev/null | python3 -c "import sys,json;print(json.load(sys.stdin).get('l2_hit',0))" 2>/dev/null || echo 0)
for i in 1 2 3; do
...
L2H1=$(dget /tmp/rq_s1.$$ cache | python3 -c "import sys,json;print(json.load(sys.stdin).get('l2_hit',0))" 2>/dev/null || echo 0)
L2E=$(dget /tmp/rq_s1.$$ cache | python3 -c "import sys,json;print(json.load(sys.stdin).get('l2_entries',0))" 2>/dev/null || echo 0)
DELTA=$((L2H1 - L2H0))
if [ "$DELTA" -ge 2 ] && [ "$L2E" -ge 1 ]; then
  ok "L2: l2_hit +$DELTA,l2_entries=$L2E(embedding 已复用)"
else
  bad "L2 行为不符: l2_hit $L2H0→$L2H1,l2_entries=$L2E"
fi
...
# ── 5. epoch 失效: upload 后同 query 立即 miss ──
say ""
say "[5] epoch 失效: upload 后同 query 立即 cache_hit=false"
curl -s -o /dev/null -X POST "$BASE/api/rag/upload/pdf" -H "X-API-Key: $KEY" -F "file=@docs/COMPLIANCE.md"
CODE=$(ask "$Q")
CH=$(dget /tmp/rq_body.$$ cache_hit)
if [ "$CH" = "False" ]; then
