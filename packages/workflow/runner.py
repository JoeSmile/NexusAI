...
Patch the consumer, not only the defining module, e.g.::
    ...
    patch("packages.workflow.run_lifecycle.rebuild_tenant_context", ...)
    patch("packages.workflow.node_exec.invoke", ...)
    patch("packages.workflow.runner_shared.write_audit_sync", ...)

Patching ``runner_shared.rebuild_tenant_context`` alone does **not** affect
``run_lifecycle`` code that already imported the name.
``from packages.workflow.runner import start_run`` etc. still works.
"""
...
from packages.workflow.node_exec import (
...
from packages.workflow.run_lifecycle import (
    cancel_run,
    continue_run,
    execute_run,
    resume_run,
...
from packages.workflow.run_maintenance import mark_zombie_runs_failed
from packages.workflow.run_parent import (
