...
async def model_router(state: PipelineState) -> PipelineState:
    ...
    if state.get("triggered_run") or state.get("finish_reason") in {
        "workflow_triggered",
        "skill_executed",
    }:
    ...
    from packages.pipeline.short_path import (
        execute_skill_short_path,
        should_take_skill_short_path,
    )

    skill = resolve_short_path_skill(state)
    state["short_path_skill"] = skill_to_state(skill)

    if should_take_skill_short_path(state):
    ...
    from packages.thread_pool import run_in_embed_pool

    if await run_in_embed_pool(try_apply_semantic_cache, state):
        enrich_span(
