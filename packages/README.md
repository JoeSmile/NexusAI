...
进程入口见 `apps/`。未搬：`backend/core` 其余散文件（`config`、`metrics`、LLM key 族…）。已迁散文件：`packages.errors`、`packages.audit*`、`packages.redis_tools`、`packages.rate_limiter`、`packages.token_quota`、`packages.cost_manager`、`packages.text_normalize`。
