...
进程入口见 `apps/`。未搬：`backend/core` 其余散文件（`rate_limiter`、`config`…）。已迁散文件：`packages.errors`、`packages.audit`、`packages.audit_context`、`packages.audit_org_scope`、`packages.redis_tools`。
