...
DEFAULT_PROMPT = (
    "请仔细观察这张图片，输出一份**准确、具体、可被下游直接使用**的描述。\n"
    "通用要求：先说这张图是什么（类型/场景/主体），再按需展开细节；不确定的内容明确标'看不清楚'，不要猜。\n"
    "分场景：\n"
    "- 若是界面截图：列出界面元素与按钮，逐字转写可见报错与文案（报错原文加引号），说明整体布局。\n"
    "- 若是图表/数据图：说明坐标含义、趋势走向、关键数值与异常点，结论放最后，数值照抄不估算。\n"
    "- 若是文档/书页照片：说明版式与结构（标题层级/段落/表格），逐字转写正文关键内容。\n"
    "- 若是营销素材（海报/口播封面/直播背景）：描述视觉主体、文案内容、色调风格、面向人群的观感。\n"
    "结尾给一句'这张图最值得注意的点'（如显眼的错误、核心数据、可复用的设计）。"
)
