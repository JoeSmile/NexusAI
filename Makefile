...
verify:
	$(require_uv)
	@echo "== brand =="
	@grep -rE "心语|情感陪伴|xinyu|emotional_chat|emotional-chat|温暖耐心|我理解你的感受|倾听你的感受|Psychology|EMOTIONAL_CHAT" $(ROOT_DIR)/backend/ $(ROOT_DIR)/scripts/ $(ROOT_DIR)/frontend/ --include="*.py" --include="*.sh" --include="*.html" --include="*.js" && echo "❌ brand" && exit 1 || echo "✅ 无情感化字眼"
	@echo "== project name =="
