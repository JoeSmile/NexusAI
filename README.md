...
**项目层级：全栈自研**（后端 FastAPI + 前端 React + 意图识别模型自训 + **基于 LangGraph 的自研编排层**：Planner→Supervisor 中心化编排 / 并行组 / on_fail 语义 / L2 漂移）。
...
| v1.1（已完成） | 中心化编排运行时（并行子 Agent + 结构化结论回传）· AgentType 注册表（槽位门 + spawn）· 澄清机制闭环 · coref 指代消解 · 意图模型 V1.0 接入 · 渠道 webhook 骨架（企微/飞书签名验签待实现） |
